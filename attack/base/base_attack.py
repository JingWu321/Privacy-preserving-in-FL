...
class _BaseAttacker:
    ...
    def prepare_attack(self, server_payload, shared_data):
        ...
        self.dm = torch.as_tensor(server_payload["data"].mean, **self.setup)[None, :, None, None]  # 1xCx1x1
        ...
        shared_data = self._cast_shared_data(shared_data)  # as list
    ...
    def _cast_shared_data(self, shared_data):
        ...
        for shared_grad in shared_data["gradients"]:
            cast_grad_list += [[g.to(dtype=self.setup["dtype"]) for g in shared_grad]]  # cast_grad_list[0]: whole gradients as list
