...
def mse(img_batch, ref_batch, batched=False):
    def get_mse(img_in, img_ref):
        mse = ((img_in - img_ref)**2).mean()
        return mse

    if batched:
        mse = get_mse(img_batch.detach(), ref_batch)
    else:
        [B, C, m, n] = img_batch.shape
        mses = []
        for sample in range(B):
            mses.append(get_mse(img_batch.detach()[sample, :, :, :], ref_batch[sample, :, :, :]))
        mse = torch.stack(mses, dim=0).mean()
    return mse
